...
class Penjualan(models.Model):
    ...
    detailpenjualan_id = fields.One2many(
        comodel_name='diegomart.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for line in self:
            result = sum(self.env['diegomart.detailpenjualan'].search(
    ...
    def __ondelete_penjualan(self):
        if self.detailpenjualan_id:
            penjualan = []
            for line in self:
                penjualan = self.env['diegomart.detailpenjualan'].search(
    ...
    def write(self, vals):
      for line in self:
          data_asli = self.env['diegomart.detailpenjualan'].search([('penjualan_id', '=', line.id)])
          print(data_asli)
      ...
      for line in self:
          data_setelah_edit = self.env['diegomart.detailpenjualan'].search([('penjualan_id', '=', line.id)])
          print(data_asli)
...
class DetailPenjualan(models.Model):
    _name = 'diegomart.detailpenjualan'
    _description = 'Detail'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(
        comodel_name='diegomart.penjualan',
        string='Detail Penjualan')
    barang_id = fields.Many2one(
        comodel_name='diegomart.barang',
        string='List Barang')
    ...
    def create(self, vals):
        ...
        if line.qty:
            self.env['diegomart.barang'].search(
